...
for source in sources:
    ...
    flattened_data = pd.read_csv(f'./c8y/meas_csv_results/{bike_number}.csv', index_col='time', low_memory=False)
    ...
    for value_to_plot_tuple in values_to_plot:
        try:
            ...
            plt.title(f'Avg. {value_to_plot_name} over the last {number_of_days} days of activity for {bike_number}')
            plt.xlabel('Time')
            plt.ylabel(f'{value_to_plot_name} ({unit_for_value})')
            plt.grid(True) # Enable the grid
            plt.xticks(rotation=45) # Adjust the rotation angle as needed

            output_folder = f'./c8y/plots/{bike_number}'
            ...
            print(f'Ran into an error while plotting data for {bike_number}: {e}')